...
rd -= np.mean(rd)
dx = 1./N
A = np.asmatrix(np.zeros((N,N)),dtype=double) # mat turns the numpy array into an object of matrix type
ip1 = (np.arange(N) + 1) # define arrays that are the indeces i+1 and i-1
ip1[N-1] = 0
im1 = (np.arange(N) - 1)
im1[0] = N-1
for i in range(0,N):
    ...
    A[i,im1[i]] = 1
    A[i,ip1[i]] = 1
b = np.asmatrix(rd,dtype=double).T*C # *dx**2  do this small number later
